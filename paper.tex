...
\begin{tabular}{lrr}
\toprule
Operation & mahotas & pymorphy\\
\midrule
erode       &     9.90 &    13.06 \\
dilate      &    10.48 &     8.17 \\
open        &    20.26 &    21.05 \\
cwatershed  &   181.47 & 50211.56 \\
center mass &     6.39 &       NA \\
daubechies  &    16.92 &       NA \\
haralick    &   276.90 &       NA \\
\bottomrule
\end{tabular}
\caption{Efficiency Results for mahotas and pymorph. Shown are values as
multiples of the time that \code{numpy.max(f)} takes.}
...
implementation of some of the same functions. We can see that simple
morphological operations (erosion, dilation, opening) take a similar amount of
time in mahotas and pymorph (pymorph intelligently uses numpy operations for
these). However, some more complex operations such as watershed are much faster
in mahotas.
