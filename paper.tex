...
\font\CodeFont="DejaVu Sans Mono" at 8pt
\setmainfont[Mapping=tex-text]{Linux Libertine O}
\newcommand*{\cpp}{{C\nolinebreak[4]\hspace{-.05em}\raisebox{.4ex}{\tiny\textbf{++}}}}
\def\code#1{\bgroup\CodeFont #1\egroup}
...
\footnote{Note for reviewers: the version currently available (0.9.6)
...
The majority of functions in mahotas have an implementation in \cpp, and a
hand-written Python wrapper. Automatically generated wrappers inevitably lead
to worse error messages and are less flexible.
...
specialisation of the template that does the actual work. These macros will
also take care of catching any \cpp{} exceptions and converting them to Python
exceptions. In this example, \code{erode} implements erosion:
...
Among the other libraries, mahotas is the fastest, except for the case of a
large median filter. For simple operations, pymorph intelligently uses
arithmetic operations for this operation and can be very fast. However, for
more complex methods, such as watershed; its pure Python approach is very
inefficient. Skimage also uses compiled code to achieve fast results.
...
Mahotas includes a complete automated suite of unit tests. These test all
...
been downloaded over 35,000~times. This does not include any downloads from
